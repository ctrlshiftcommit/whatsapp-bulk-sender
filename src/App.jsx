...
function App() {
  ...
  return (
    <div className="flex min-h-screen bg-mist">
      ...
      {modal === "import" && <ImportModal groups={groups} onClose={() => setModal(null)} onImport={async (payload) => { try { const settings = await api.getSettings(); const added = await api.importContacts({ ...payload, defaultCountryCode: String(settings.country || "91").replace(/\D/g, "") }); setContacts(await api.listContacts()); setGroups(await api.listGroups?.() || []); setSummary(await api.getDashboardSummary()); setModal(null); notify(`${added.length} contacts imported`); } catch (error) { reportError(error, "Could not import contacts"); } }} onImportFile={() => { setImportPreview(null); setModal("import-map"); }} />}
      {modal === "import-map" && <ImportMapModal preview={importPreview} groups={groups} onClose={() => { setModal(null); setImportPreview(null); }} onChooseFile={async () => { try { const preview = await api.previewContactFile?.(); if (preview) setImportPreview(preview); } catch (error) { reportError(error, "Could not open contact file"); } }} onImport={async (payload) => { try { if (!importPreview?.source) throw new Error("Choose a CSV or Excel file before importing."); const settings = await api.getSettings(); const added = await api.importContactFile({ ...importPreview, ...payload, defaultCountryCode: String(settings.country || "91").replace(/\D/g, "") }); setContacts(await api.listContacts()); setGroups(await api.listGroups?.() || []); setSummary(await api.getDashboardSummary()); setModal(null); setImportPreview(null); notify(`${added.length} contacts imported`); } catch (error) { reportError(error, "Could not import contacts"); } }} />}
...
function CampaignWizard({ contacts, groups, templates, media, connection, onClose, onLaunch }) {
  ...
  const nextLabel = step === 1 ? "Choose recipients" : step === 6 ? (isScheduled ? "Schedule campaign" : "Launch campaign") : "Continue";
  return <Modal wide title="Create campaign" sub="Build a careful, opt-in send in six steps." onClose={onClose}><div className="mb-6 flex items-center">{steps.map((label, i) => <button key={label} type="button" onClick={() => setStep(i + 1)} className="flex flex-1 items-center text-left"><div className={cx("flex h-7 w-7 items-center justify-center rounded-full text-[11px] font-bold", step >= i + 1 ? "bg-emerald text-white" : "bg-slate-100 text-slate-400")}>{step > i + 1 ? <Check size={14} /> : i + 1}</div><span className={cx("ml-2 text-[11px] font-semibold", step === i + 1 ? "text-slate-800" : "text-slate-500")}>{label}</span>{i < 5 && <div className="mx-2 h-px flex-1 bg-slate-200" />}</button>)}</div>
    ...
    <div className="mt-6 flex justify-between border-t border-slate-100 pt-4"><button onClick={step === 1 ? onClose : () => setStep(step - 1)} className="btn-secondary">{step === 1 ? "Cancel" : "Back"}</button><button onClick={() => step === 6 ? submitCampaign() : setStep(step + 1)} disabled={step === 6 && !canSubmit} className={cx("btn-primary", step === 6 && !canSubmit && "cursor-not-allowed opacity-50")}>{nextLabel} <ChevronRight size={15} /></button></div>
  </Modal>;
}
