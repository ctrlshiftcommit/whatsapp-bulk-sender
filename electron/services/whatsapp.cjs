  ...
  async sendMessage(contact, { message, mediaPath, typingSimulation }) {
    ...
    try {
      ...
      await this.page.waitForFunction(() => {
        ...
        const error = document.querySelector('[data-testid="popup-contents"], div[role="dialog"]');
        return input || error;
      }, { timeout: 20000 });
      const invalid = await this.page.$('[data-testid="popup-contents"], div[role="dialog"]');
      if (invalid) throw new Error(`Number +${phone} is not available on WhatsApp.`);
